    ...
    <div className="max-w-6xl mx-auto px-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold text-gray-900">Check-in de Convidados</h1>
          {events.length > 0 && (
            <select
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-yellow-50 rounded-lg p-4 text-center">
            <p className="text-3xl font-bold text-yellow-600">{stats.confirmados}</p>
      ...
      {selectedEventId && (
        <>
          {/* Scanner de QR Code */}
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Scanner QR Code</h2>
            ...
            {!isScanning ? (
              ...
              <div>
                ...
                <div id="qr-reader" className="mx-auto" style={{ maxWidth: '500px' }}></div>
              </div>
            )}
            ...
            {lastCheckin && (
              <div className={`mt-6 p-4 rounded-lg ${
                lastCheckin.alreadyCheckedIn ? 'bg-yellow-50 border-yellow-200' : 'bg-green-50 border-green-200'
              } border`}>
                <p className="font-medium text-gray-900 text-lg">
                  {lastCheckin.alreadyCheckedIn ? '⚠️' : '✅'} {lastCheckin.nome}
                </p>
                <p className="text-gray-600">Mesa {lastCheckin.mesa}</p>
                {lastCheckin.alreadyCheckedIn && (
                  <p className="text-sm text-yellow-700 mt-1">Já tinha feito check-in anteriormente</p>
                )}
              </div>
            )}
          </div>
          ...
          {/* Check-in Manual */}
          <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Check-in Manual</h2>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Últimos Check-ins</h2>
            {checkinHistory.length > 0 ? (
              <div className="space-y-2">
                {checkinHistory.map((guest) => (
                  <div key={guest.id} className="flex justify-between items-center py-2 border-b">
                    <div>
              ...
              <p className="text-gray-500 text-center py-4">Nenhum check-in realizado ainda</p>
            )}
          </div>
        </>
      )}
